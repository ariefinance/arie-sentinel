...
| UK | UK Sanctions List on GOV.UK (`Designation` records) | XML | Yes |
...
**The UK OFSI Consolidated List was retired on 28 Jan 2026**; Sentinel therefore uses the
**UK Sanctions List** on GOV.UK. The retired OFSI `ConsolidatedList` element layout is also
accepted by the parser for continuity.
...
One free credential activates the fully-built UK Companies House adapter
(`ARIE_COMPANIES_HOUSE_API_KEY`). Sanctions feeds need no key (EU FSF is optional and left
unconfigured by default). Never commit any key. Schedule
`python -m arie_sentinel.jobs.refresh_sanctions` (cron / systemd timer) so the sanctions
cache stays fresh; screening treats a stale/missing required feed as a coverage limitation,
not a clearance.
